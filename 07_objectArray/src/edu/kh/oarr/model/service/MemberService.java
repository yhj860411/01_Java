...
public class MemberService {
	...
	public void displayMenu() {
		...
		do {
			...
			default : System.out.println("잘못 입력하셨습니다. 메뉴에 있는 번호만 선택하세요.");
			}
	...
	public int emptyIndex() {
		for(int i = 0; i < memberArr.length; i++) {
			...
			if(memberArr[i] == null) {
				return i;
				// 현재 메서드를 종료하고 호출한 곳으로 i값을 가지고 되돌아감
			}
	...
	public String login() {
		...
		for(int i = 0; i < memberArr.length; i++) {
			...
			if(memberArr[i] != null) {// 회원 정보가 있을 경우
				// 2-2) 회원 정보가 있을 경우 -> 회원정보의 아이디, 비밀번호와
				// 입력받은 (로그인)아이디, 비빌번호가 같은지 비교
				if(memberArr[i].getMemberId().equals(memberId) 
						&& memberArr[i].getMemberPw().equals(memberPw)) {
					// 2-3) 아이디/비밀번호 같다면
					// 로그인 회원 정보 객체를 참조할 변수 loginMember에
					// 현재 접근중인 memberArr[i] 요소에 저장된 주소를 얕은복사
					loginMember = memberArr[i];
					break; // 더이상 같은 아이디, 비밀번호가 없을것이라고 예상.
					// 효율을 위해 for문 종료
				}
			}
		}// for문 종료
