...
public class ArrayExample1 {
	...
	public void ex2() {
		...
		// 배열의 인덱스 범위를 벗어남 :
		// 길이를 4칸짜리로 만든 배열인데, 없는 인덱스번호인 4에 접근했다
	...
	public void ex4() {
		...
		System.out.printf("평균 : %.1f", sum/(double)person);
	}
	...
	public void ex7() {
		...
		for(int i = 0; i < arr.length; i++) {
			if(input.equals(arr[i])) { // String 비교시 equals() 사용!
				System.out.println((i+1) + "번 인덱스에 존재");
				flag = true; // 찾았다고 표시
			}
		}
		if(!flag) {
	...
	public void ex8() {
		...
		// 1-2) 입력한 문자열을 한글자씩 잘라내어 char 배열의 모든 요소에 순서대로 대입하기
		for(int i = 0; i< arr.length; i++) {
		...
		// String으로 우선 입력받고, charAt(index) 메서드를 이용하여 해당 index번째 존재하는
		// 문자 하나를 떼어 char 자료형 형태로 반환함.
		
		// arr이라는 배열에 입력한 문자가 몇개 존재하는지
		// 0번 index부터 마지막 index까지 순회하며 검색
